...
pub struct TranscriptionManager {
    ...
    active_engine_lease: Arc<AtomicU64>,
    /// DotFlow: tracks committed text already injected into the focused field for the streaming
    /// field-injection path (experimental_field_streaming). Reset at each stream start.
    field_streamer: Arc<Mutex<crate::dotflow::FieldStreamer>>,
    /// DotFlow: wall-clock of the last field injection, used to THROTTLE streaming writes. Rapid
    /// back-to-back `enigo` writes overlap in the OS input queue and cause dropped/repeated keys
...
impl TranscriptionManager {
    ...
    pub fn start_stream(&self) {
        ...
        self.stream_active.store(false, Ordering::Release);
        // DotFlow: fresh field-streaming tracker + throttle clock for this dictation.
        self.field_streamer.lock().unwrap().reset();
        *self.last_field_inject.lock().unwrap() = None;
    ...
    fn emit_stream_text(&self, committed: &str, tentative: &str) {
        ...
        // DotFlow: stream the COMMITTED text into the FOCUSED FIELD as it grows (experimental_field_streaming,
        // default off). Word-batched + append-only + phrase-aware: `advance` releases newly-COMPLETED words
        ...
        if settings.experimental_field_streaming {
            ...
            // advancing — the newly completed words stay in `committed` and are released together on the next
            // (spaced) write, so nothing is lost and consecutive enigo bursts never overlap. The overlay
            // above still updates every tick, and the held tail flushes on stop.
            let throttle = std::time::Duration::from_millis(settings.field_stream_throttle_ms);
            if let Some(t) = *self.last_field_inject.lock().unwrap() {
    ...
    /// DotFlow: on stop, sync the field to the FINAL committed text — `flush` releases the held trailing word
    /// (and any last uncommitted words), then reset. Call this instead of the normal paste when
