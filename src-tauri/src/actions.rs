                ...
                if samples.is_empty() {
                    ...
                    match transcription_result {
                        Ok(transcription) => {
                            ...
                            // DotFlow: field-streaming injected the committed text into the field LIVE during
                            // the stream. Sync the field to the final text (types any last uncommitted words)
                            // and SKIP the normal paste (it would duplicate the whole utterance).
                            if get_settings(&ah).experimental_field_streaming {
                                tm.finalize_field_stream(&transcription);
                                utils::hide_recording_overlay(&ah);
                                change_tray_icon(&ah, TrayIconState::Idle);
                                return;
                            }
