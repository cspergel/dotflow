...
pub fn catalog() -> Vec<LlmModelInfo> {
    vec![
        LlmModelInfo {
            ...
            commercial_ok: true,
            recommended: true,
            note: "Google's latest small instruct model (Apache-2.0). Recommended default — strong quality, commercially usable, runs on the offline engine."
                .to_string(),
        ...
        LlmModelInfo {
            ...
            commercial_ok: true,
            recommended: false,
            note: "Clean (Apache-2.0), fast, and a small download. A reliable, lightweight alternative — works with the current offline engine."
                .to_string(),
...
mod tests {
    ...
    fn catalog_is_well_formed() {
        ...
        // non-commercial model) AND must load on the bundled engine. Currently Gemma 4 E2B (Apache-2.0),
        // which loads on llama-cpp-2 0.1.151+ (the `gemma4` architecture landed in 0.1.146).
