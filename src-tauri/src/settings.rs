...
impl Default for PasteMethod {
    fn default() -> Self {
        // DotFlow: injection-FIRST on every platform (design §11a "inject, don't paste"). `Direct` uses
        // enigo.text() keystroke injection, so text pops into the field in its own formatting without
        // touching the clipboard — the Dragon feel. (Handy defaulted to clipboard Ctrl+V on Win/mac.)
        // Clipboard-paste remains a manual choice for fields where injection is flaky (the design's
        // required "reliable paste" fallback), and huge-block auto-fallback is a later refinement.
        PasteMethod::Direct
    }
}
...
pub struct AppSettings {
    ...
    /// DotFlow: stream a STREAMING model's committed text into the focused field as you speak (the true
    /// Dragon feel). Default OFF. Requires a streaming model (Parakeet Unified / Nemotron). When on, the
    /// committed text is keystroke-injected incrementally (the tentative guess stays in the overlay).
    #[serde(default)]
    pub experimental_field_streaming: bool,
    /// DotFlow: minimum spacing (ms) between streaming field-injection writes (used only when
    /// `experimental_field_streaming` is on). Words completed within the window accumulate and flush in the
    /// next write, so consecutive keystroke bursts never overlap in the OS input queue (dropped/repeated
    /// keys). Higher = safer against the race but text appears in slightly larger, less frequent chunks.
    #[serde(default = "default_field_stream_throttle_ms")]
    pub field_stream_throttle_ms: u64,
    /// DotFlow: per-character delay (ms) when keystroke-injecting text into the field. Raise it if injected
    /// text shows dropped or repeated characters (the OS missing key-ups under fast typing); lower it for
...
fn default_model() -> String {
    // DotFlow: default to NVIDIA Parakeet (TDT 0.6B v3, ONNX/CPU via transcribe-rs). Handy shipped this empty
    // (onboarding picks a model); DotFlow ships Parakeet as the out-of-box ASR.
    "handy-computer/parakeet-tdt-0.6b-v3-gguf".to_string()
}
...
/// DotFlow: default streaming field-injection throttle (ms). 100ms spaces the enigo bursts enough to avoid
/// the OS-input-queue race on typical machines while still feeling live; tunable via settings.
fn default_field_stream_throttle_ms() -> u64 {
    100
}

/// DotFlow: default per-character keystroke-injection delay (ms). 8ms lets each key-down/up land so the OS
/// never auto-repeats a key; tunable per machine via settings.
fn default_field_stream_char_delay_ms() -> u64 {
    8
}
