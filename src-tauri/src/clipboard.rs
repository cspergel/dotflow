...
/// DotFlow: inject raw text via keystroke injection (`enigo.text()`), no clipboard, and WITHOUT the wedge
...
pub fn inject_text_raw(text: &str, app_handle: &AppHandle) -> Result<(), String> {
    ...
    #[cfg(target_os = "linux")]
    {
        paste_direct(&mut enigo, text, get_settings(app_handle).typing_tool)
    }
    #[cfg(not(target_os = "linux"))]
    {
...
// DotFlow: on some Windows setups `enigo.text()` sends Unicode key events faster than the OS/target app
// drains them, so a key-up is missed and the key auto-repeats ("ggggg"). Typing one character at a time with
// a small per-char gap (the `field_stream_char_delay_ms` setting) gives each key-down/up pair time to land —
// the definitive fix (throttling whole CALLS can't help; the race is INSIDE a single multi-char text() call).

/// Type `text` one character at a time with `char_delay_ms` between characters, holding the given enigo lock
/// throughout so no other injection can interleave. The reliable, clipboard-free insertion primitive.
fn type_text_char_by_char(enigo: &mut Enigo, text: &str, char_delay_ms: u64) -> Result<(), String> {
    let mut buf = [0u8; 4];
...
/// DotFlow: apply a STREAMING field edit — press Backspace `backspaces` times, then type `text` — via enigo
/// keystroke injection (no clipboard). Used by the streaming field-injection path to keep the committed text
/// in the focused field in sync as a streaming model emits/revises it. Normally `backspaces == 0` (committed
...
pub fn inject_field_edit(backspaces: usize, text: &str, app_handle: &AppHandle) -> Result<(), String> {
    ...
    type_text_char_by_char(&mut enigo, text, char_delay_ms)?;
    Ok(())
}
...
pub fn paste(text: String, app_handle: AppHandle) -> Result<(), String> {
    ...
    // DotFlow: the deterministic wedge — expand phrases (`.fix` / "insert follow up") and apply the
    // punctuation mode BEFORE injection (design §11c "parse-and-expand before inject"). Auto punctuation
